...
# --- UI minimalissima in un'unica rotta ---
from fastapi.responses import HTMLResponse  # <— se già presente, lascia com'è

@app.get("/admin/ui", response_class=HTMLResponse)
def admin_ui_single_page():
...
const $=(s,el=document)=>el.querySelector(s);const esc=s=>s?.replace(/[&<>"]/g,m=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;"}[m]))||"";
async function j(u,o={}){const r=await fetch(u,{...o,credentials:'include'});if(!r.ok)throw new Error(u+' -> '+r.status);return r.json();}
async function boot(){ $('#hint').textContent='Carico…'; const [c,a,t]=await Promise.all([j(api.clients),j(api.accounts),j(api.tokens)]); state.clients=c;state.accounts=a;state.tokens=t; renderList(); $('#hint').textContent='Pronto'; }
function renderList(f=''){ const box=$('#list'); box.innerHTML=''; const q=(f||'').trim().toLowerCase();
  const items=state.clients.filter(c=>{const s=`${c.id||''} ${c.name||''} ${c.company||''}`.toLowerCase(); return !q||s.includes(q);});
  for(const c of items){ const acc=state.accounts.find(a=>a.client_id===c.id); const el=document.createElement('div'); el.className='item'+(state.selected===c.id?' active':'');
...
async function select(clientId){ state.selected=clientId; renderList($('#q').value||''); const c=state.clients.find(x=>x.id===clientId); const acc=state.accounts.find(a=>a.client_id===clientId);
  $('#crumb').textContent=c?.name||c?.company||('Cliente #'+clientId); $('#hint').textContent='Carico scheda…';
  let prompts=null,logs=[]; try{prompts=await j(api.prompts(clientId));}catch{} try{logs=await j(api.logs+`?client_id=${clientId}&limit=30`);}catch{}
  const toks=state.tokens.filter(t=>t.ig_account_id===acc?.id); renderDetail({c,acc,toks,logs,prompts}); $('#hint').textContent='Pronto';
}
function renderDetail({c,acc,toks,logs,prompts}){ const d=$('#detail'); d.innerHTML=`
  <div class="card">
    <h3>Account</h3>
    <div class="row">
    ...
    <div class="row">
      ...
      <span>Bot abilitato</span><span id="bots" style="color:var(--muted)"></span>
    </div>
  ...
  <div class="card">
    ...
    <div class="row"><button id="savep" ${prompts===null?'disabled':''}>Salva prompt</button><span id="ps" style="color:var(--muted)"></span></div>
  ...
  if(bot&&acc){ bot.onchange=async()=>{ try{bots.textContent='…'; await fetch('/admin/accounts',{method:'PATCH',credentials:'include',headers:{'Content-Type':'application/json'},body:JSON.stringify({ig_user_id:acc.ig_user_id,bot_enabled:bot.checked})}); bots.textContent='Salvato';}catch(e){bots.textContent='Errore';} }; }
  const savep=$('#savep'), ps=$('#ps'); if(savep){ savep.onclick=async()=>{ try{ps.textContent='…'; await fetch(api.prompts(c.id),{method:'PUT',credentials:'include',headers:{'Content-Type':'application/json'},body:JSON.stringify({greeting:$('#greet').value,fallback:$('#fallback').value,handoff:$('#handoff').value,legal:$('#legal').value})}); ps.textContent='Salvato';}catch(e){ps.textContent='Errore';} }; }
}
